...
public class Solution {
    ...
    static String organizingContainers(int[][] container) {
        long[] ColArray = new long[container.length];
        long[] RowArray = new long[container.length];

        long colBalls = 0;
        long rowBalls = 0;
        for (int i=0;i<container.length;i++){
            for (int j=0;j<container.length;j++){
                colBalls = colBalls + container[i][j];
                RowArray[j] = RowArray[j] + container[i][j];
            }
            ColArray[i] = colBalls;
            colBalls = 0;
            System.out.print("RowArray ");
            printArray(RowArray);
            System.out.print("ColArray ");
            printArray(ColArray);
            System.out.println();
        ...
        for (int i=0;i<container.length;i++){
            System.out.println("RowArray[i] " + RowArray[i] +", ColArray[i] " + ColArray[i]);
        }
        System.out.println("=======");
